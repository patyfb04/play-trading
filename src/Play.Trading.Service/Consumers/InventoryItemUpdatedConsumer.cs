    ...
    public class InventoryItemUpdatedConsumer : IConsumer<InventoryItemUpdated>
    {
        ...
        public async Task Consume(ConsumeContext<InventoryItemUpdated> context)
        {
           var message = context.Message;
            ...
            var intentoryItem =  await _repository.GetAsync(item => 
                    item.UserId ==  message.UserId && 
                    item.CatalogItemId == message.CatalogItemId);

            if (intentoryItem is null)
            {
               intentoryItem = new InventoryItem
               {
                ...
                await _repository.CreateAsync(intentoryItem);
            }
            else
            {
                intentoryItem.Quantity = message.NewTotalQuantity;
                 await _repository.UpdateAsync(intentoryItem);
            }
        }
    }
}
